...
class ConwayCube(object):
    ...
    def __init__(self, initial_state):
        ...
        self.cube = np.full((self.side_len, self.side_len, self.side_len), self.inactive, dtype="S1")
        self.cube[1] = initial_state
        
    def num_active_around(self, i, j, k):
        """
        Number of active sites around the point (i, j)
        """
        surrounding_cube = self.cube[max(0, i-1):i+2, max(0, j-1):j+2, max(0, k-1):k+2]
        active_around = (surrounding_cube == self.active).sum()
        if self.cube[i, j, k] == self.active:
            return active_around - 1
        else:
            return active_around
        
    def next_state(self, i, j, k):
        if self.cube[i, j, k] == self.active:
            if self.num_active_around(i, j, k) in [2,3]:
                return self.active
            else:
                return self.inactive
        else:
            if self.num_active_around(i, j, k) == 3:
                return self.active
            else:
                return self.inactive
    ...
    def step(self):
        ...
        embedded_cube = np.full((self.side_len, self.side_len, self.side_len), self.inactive, dtype="S1")
        embedded_cube[1:self.side_len-1, 1:self.side_len-1, 1:self.side_len-1] = self.cube
        ...
        for i in range(self.side_len):
            for j in range(self.side_len):
                for k in range(self.side_len):
                    next_cube[i, j, k] = self.next_state(i, j, k)
        self.cube = next_cube
