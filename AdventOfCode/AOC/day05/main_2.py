...
with open("input.txt") as f:
    present_seats = {i:False for i in range(128 * 8)}
    for line in f.readlines():
        line = line.rstrip()
        present_seats[get_location(line)[2]] = True

where_false = list(filter(lambda x: not present_seats[x], present_seats.keys()))
for i in where_false:
    if i-1 not in where_false and i+1 not in where_false:
        print(i)
