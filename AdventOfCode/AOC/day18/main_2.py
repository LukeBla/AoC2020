...
def split_lst(lst):
    # Get first "*" and evaluate left/right
    try:
        mult_i = lst.index("*")
    except ValueError:
        mult_i = None
    if mult_i is None:
        ...
        return (lst[:mult_i], lst[mult_i], lst[mult_i+1:])
...
class ExpressionTree(object):
    def __init__(self, left, op, right):
        ...
        if isinstance(left, list):
            # Get first multiplication
            (l, o, r) = split_lst(left)
            left = ExpressionTree(l, o, r)
        if isinstance(right, list):
            (l, o, r) = split_lst(right)
            right = ExpressionTree(l, o, r)
        self.left = left
        self.right = right
        self.op = op
...
def get_tree(multi_list):
    (l, o, r) = split_lst(multi_list)
    return ExpressionTree(l, o, r)
